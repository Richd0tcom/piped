...
func (s *Store) migrate() error {
	_, err := s.db.Exec(`
		...
		CREATE TABLE IF NOT EXISTS log_lines (
			...
			deployment_id VARCHAR(31) NOT NULL,
...
func (s *Store) CreateDeployment(d *models.Deployment) error {
	...
	_, err := s.db.Exec(`
		INSERT INTO deployments (id,name,source_type,git_url,git_branch,git_commit,env_vars,status,created_at,updated_at)
		VALUES (?,?,?,?,?,?,?,?,?,?)`,
		d.ID, d.Name, d.SourceType, d.GitURL, d.GitBranch, d.GitCommit, string(env),
...
func (s *Store) ListDeployments() ([]*models.Deployment, error) {
	rows, err := s.db.Query(`SELECT * FROM deployments ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) GetLogs(deploymentID string, fromSequence int64) ([]*models.LogLine, error) {
	rows, err := s.db.Query(`
...
func scanDeployment(row scanner) (*models.Deployment, error) {
	...
	var envJSON string
	err := row.Scan(
		&d.ID, &d.Name, &d.SourceType, &d.GitURL, &d.GitBranch, &d.GitCommit, &envJSON,
		&d.Status, &d.ImageTag, &d.ActiveContainerID, &d.StandbyContainerID,
